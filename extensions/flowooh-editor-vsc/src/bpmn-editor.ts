...
/**
 * Define the delegate for paw draw files.
 */
...
class BpmnDocument extends Disposable implements vscode.CustomDocument {
  ...
  private constructor(uri: vscode.Uri, initialText: string, delegate: BpmnDocumentDelegate) {
    super();
    this._uri = uri;
  ...
  private readonly _onDidRename = this._register(
    new vscode.EventEmitter<{
      oldUri: vscode.Uri;
      newUri: vscode.Uri;
    }>(),
  ...
  makeEdit(edit: BpmnEdit) {
    ...
    const [lastEdit = { idx: -1 }] = this._edits.slice(-1);
...
export class BpmnEditor implements vscode.CustomEditorProvider<BpmnDocument> {
  ...
  public static register(context: vscode.ExtensionContext): vscode.Disposable {
    context.subscriptions.push(
      vscode.commands.registerCommand(`${BpmnEditor.viewType}.new`, async () => {
        const currentDocumentUri = vscode.window.activeTextEditor?.document.uri || vscode.window.activeNotebookEditor?.notebook.uri;
    ...
    return vscode.window.registerCustomEditorProvider(BpmnEditor.viewType, new BpmnEditor(context), {
      webviewOptions: {
  ...
  private readonly log = vscode.window.createOutputChannel('Flowooh Editor');

  private static readonly viewType = 'flowooh-editor';
  ...
  constructor(private readonly _context: vscode.ExtensionContext) {
    _context.subscriptions.push(
      vscode.commands.registerCommand(`${BpmnEditor.viewType}.__state`, (uri: vscode.Uri) => {
  ...
  // override CustomEditorProvider methods

  async openCustomDocument(uri: vscode.Uri, openContext: { backupId?: string }, _token: vscode.CancellationToken): Promise<BpmnDocument> {
  ...
  async resolveCustomEditor(document: BpmnDocument, webviewPanel: vscode.WebviewPanel, _token: vscode.CancellationToken): Promise<void> {
    ...
    webviewPanel.onDidChangeViewState(async (e) => {
      ...
      if (content !== document.getText()) {
        const action = await vscode.window.showInformationMessage('Diagram changed externally, do you want to reload it?', 'Reload');
  ...
  private getHtmlForWebview(webview: vscode.Webview): string {
    ...
    const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, 'out/client', 'bpmn-editor.js'));
    ...
    const styleMainUri = webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, 'out/client', 'bpmn-editor.css'));
  ...
  private onMessage(document: BpmnDocument, message: any) {
    switch (message.type) {
      ...
      case 'response':
        return this._callbacks.get(message.requestId)?.(message.body);
    }
  }
}
